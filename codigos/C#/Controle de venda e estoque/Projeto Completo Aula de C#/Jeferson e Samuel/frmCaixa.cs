    ...
    public partial class frmCaixa : Form
    {
        ...
        private void btnGravar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow Linha in dgvTitulos.Rows)
                {
                    if (bool.Parse(Linha.Cells[0].Value.ToString()) == true)
                    {
                        Global.Comando = new MySqlCommand("UPDATE contas_receber SET Status = ?Status, Data_Pgto = ?Data_Pgto " +
                                                          "WHERE ID_Venda = ?ID_Venda AND Parcela = ?Parcela", Global.Conexao);
                        ...
                        Global.Comando.Parameters.AddWithValue("?ID_Venda", int.Parse(Linha.Cells[1].Value.ToString()));
        ...
        private void Selecionados()
        {
            ...
            foreach (DataGridViewRow Linha in dgvTitulos.Rows)
            {
                if (bool.Parse(Linha.Cells[0].Value.ToString()) == true)
                {
                    Total += double.Parse(Linha.Cells[4].Value.ToString());
                }
            }
